...
def main(nusc, indice, nuscenesyaml, args, config):
    ...
    my_scene = nusc.scene[indice] # todo nusc.scene: 从 dataroot/version/scene.json中加载的场景信息
    sensor = 'LIDAR_TOP'
    ...
    first_sample_token = my_scene['first_sample_token'] # 当前场景第一个sample标识符
    my_sample = nusc.get('sample', first_sample_token)  # 获取sample数据
    lidar_data = nusc.get('sample_data', my_sample['data'][sensor]) # 获取LIDAR_TOP数据
    lidar_ego_pose0 = nusc.get('ego_pose', lidar_data['ego_pose_token']) # 雷达数据相对于车辆的位姿标识符
    ...
    while True:
        ############################# get boxes ##########################
        lidar_path, boxes, _ = nusc.get_sample_data(lidar_data['token']) # 获取boxes
        boxes_token = [box.token for box in boxes] # 提取物体的token
        object_tokens = [nusc.get('sample_annotation', box_token)['instance_token'] for box_token in boxes_token] # 获取物体实例和类别名称
        ...
        for category in object_category:
            for (j, label) in enumerate(nuscenesyaml['labels']):
                if category == nuscenesyaml['labels'][label]:
                    converted_object_category.append(np.vectorize(learning_map.__getitem__)(label).item()) # 将nuscenes中物体类别转换为occ类别
...
if __name__ == '__main__':
    ...
    parse.add_argument('--config_path', type=str, default='config.yaml') # todo 类别映射：用于将nuscenes中类别映射为occ类别
    parse.add_argument('--version', type=str, default='v1.0-mini')
    ...
    parse.add_argument('--label_mapping', type=str, default='nuscenes.yaml') # todo 配置文件：定义了体素大小、点云坐标范围等
    args=parse.parse_args()

    if args.dataset=='nuscenes':
        version = args.version
        nusc = NuScenes(version=version, # todo 'v1.0-mini' or 'v1.0-trainval'
    ...
    with open(args.config_path, 'r') as stream:
        config = yaml.safe_load(stream) # todo 类别映射：将nuscenes中类别映射为occ类别

    # load learning map
    label_mapping = args.label_mapping
    with open(label_mapping, 'r') as stream:
        nuscenesyaml = yaml.safe_load(stream) # todo 配置文件：定义了体素大小、点云坐标范围等

    num_scene = len(nusc.scene)

    for i in tqdm(range(num_scene)):
        main(nusc, indice=i,nuscenesyaml=nuscenesyaml, args=args, config=config)